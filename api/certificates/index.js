...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const result = await sql`SELECT * FROM public.certificates ORDER BY createdat DESC`;
    ...
    if (req.method === 'POST') {
      ...
      const existingCert = await sql`SELECT id FROM public.certificates WHERE certificatenumber = ${certificateNumber}`;
